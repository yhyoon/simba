...
let is_feasible_by_solver (grammar: Grammar.grammar) (spec: AugSpec.io_spec list) (rewrite: Grammar.rewrite): bool =
	if BatList.exists (function (_, AugSpec.OConcrete _) -> false | _ -> true) spec then begin
		Logger.g_info "not all output specs are concrete -> cannot check by smt solver";
		false
	end
	else
