...
let witness available_height nt_sigs (int_sigs, bv_sigs, string_sigs, bool_sigs) rule output_sig arg_sigs =
	...
	match op with
	| BV_OP BV_BIN_OP BV_ADD ->
		if (BatList.length arg_sigs) = 0 then
			if (is_not_fresh n_sigs op) then 
  			BatSet.filter (fun arg0_sig ->
  				let arg1_sig = fun_apply_signature (BV_OP (BV_BIN_OP BV_SUB)) [output_sig; arg0_sig] in
		...
		if (BatList.length arg_sigs) = 0 then
			BatSet.filter (fun arg0_sig ->
				BatList.for_all (fun (arg0_const, output_const) ->
					...
					(not (Int64.equal arg0_v 0L)) && (* 0 * ... = output_v *)
					(not (Int64.equal arg0_v 1L)) && (* 1 * output_v = output_v *)
					(not (Int64.equal output_v 0L)) &&  (* ... * 0 = 0 *)
					(Int64.equal (Int64.rem output_v arg0_v) 0L) &&  (* output | arg0 *)
	...
	| BV_OP BV_BIN_OP BV_SDIV ->
		if (type_of_signature output_sig) <> BV then assert false
			...
			BatSet.filter (fun arg0_sig ->
				BatList.for_all (fun (arg0_const, output_const) ->
					let arg0_v = get_bv arg0_const in
					let output_v = get_bv output_const in
					(* if (Int64.equal 0L output_v) then false *)
					(* else                                            *)
    	...
    	(* 			let arg1_v = Int64.div arg0_v output_v in                                                                                 *)
    	(* 			let _ = assert (not (Int64.equal 0L output_v)) in                                                                 *)
    	(* 			let k = Int64.rem arg0_v output_v in                                                                                      *)
    	(* 			let _ = if (Int64.equal 0L arg1_v) then                                                                           *)
	...
	| BV_OP BV_BIN_OP BV_UDIV ->
		if (BatList.length arg_sigs) = 0 then nt_sigs
			...
			(* 	let arg1_v = Int64.unsigned_div arg0_v output_v in                                                                                                                        *)
			(* 	let k = Int64.unsigned_rem arg0_v output_v in                                                                                                                             *)
			(* 	let _ = if (Int64.equal 0L arg1_v) then failwith (Printf.sprintf "%s / %s = %s" (Int64.to_string arg0_v) (Int64.to_string arg1_v) (Int64.to_string output_v) ) in *)
			(* 	acc @ [(CBV arg1_v)]                                                                                                                                                      *)
			(* ) [] (BatList.combine arg0_sig output_sig) |> BatSet.singleton                                                                                                              *)
	| BV_OP BV_BIN_OP BV_SREM ->
		if (BatList.length arg_sigs) = 0 then
			BatSet.filter (fun arg0_sig ->
				BatList.for_all (fun (arg0_const, output_const)  ->
					let arg0_v = get_bv arg0_const in
			...
			(* 		let arg0_v = get_bv arg0_const in                                                               *)
			(* 		let output_v = get_bv output_const in                                                           *)
			(* 		if (Int64.equal 0L (Int64.sub arg0_v output_v)) ||                                      *)
			(* 			 (Int64.equal 0L arg0_v) then false                                                   *)
			(* 		else                                                                                            *)
			(* 		let arg1_v =                                                                                    *)
	...
	| BV_OP BV_BIN_OP BV_ASHR ->
      ...
      (*   				if (Int64.equal output_v (arg0_v %>>% arg1_v)) then            *)
      (*   					CBV (arg0_v)                                                              *)
      ...
      (* 				)	(BatList.combine arg1_sig output_sig)                                      *)
    	...
    	(* 			BatSet.add arg0_sig acc  	                                                     *)
			...
			BatSet.filter (fun arg0_sig ->
				BatList.for_all (fun (arg0_const, output_const) ->
					...
					BatList.exists (fun i ->
						(Int64.equal output_v (arg0_v %>>% i)) &&
						(Int64.compare (Int64.mul output_v arg0_v) 0L) <> 0
  		...
  		(* 				if (Int64.compare output_v (%>>%arg0_v i)) = 0 && *)
  		(* 					 (not (Int64.equal 0L arg0_v)) then                   *)
  		(* 						(CBV arg0_v)                                                *)
    	(* 				else failwith "bvashr_arg0"                                     *)
  		(* 			) output_sig                                                      *)
  		(* 		in                                                                  *)
			...
			if (is_not_fresh n_sigs op) then 
  			BatSet.filter (fun arg1_sig ->
  				BatList.for_all2 (fun (arg0_const, arg1_const) output_const ->
  					...
  					let output_v = get_bv output_const in
						(arg1_v >= 0) && (arg1_v <= 63) &&
  					(Int64.equal (arg0_v %>>% arg1_v) output_v)
  				) (BatList.combine arg0_sig arg1_sig) output_sig
  			) nt_sigs
			else
  			BatList.fold_left (fun acc (arg0_const, output_const) ->
  				...
  				let rec iter i =
  					...
  					else if (Int64.equal output_v (arg0_v %>>% i)) then
  						Int64.of_int i
  					else iter (i + 1)
  				in
