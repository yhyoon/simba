			...
			BatSet.fold (fun pt acc ->
				if (BitSet.get cover_t pt) then
					let total_covered_pts =
						...
						if Global.t.cli_options.fast_dt then 
...
let get_best_bool_sig pts bool_nt bool_sigs nt_sigs cover pt2covers pt2covered_array =
	...
	let g_scores =
		List.map (fun bool_sig ->
			...
			in
			Logger.g_debug_f "score: %.2f" score;
			(bool_sig, score)			 
...
=
	...
	Logger.g_debug_f "minimal |nt_sigs| = %d" (BatSet.cardinal nt_sigs);
	...
	let bool_sigs =
		BatSet.filter (fun bool_sig ->
			...
			List.exists (fun x -> (Stdlib.compare x (CBool (Concrete true))) = 0) bool_sig &&
			List.exists (fun x -> (Stdlib.compare x (CBool (Concrete false))) = 0) bool_sig
		) bool_sigs
	in
	Logger.g_debug_f "|bool_sigs|: %d" (BatSet.cardinal bool_sigs);
	let nt_sigs = BatMap.find nt nt_to_sigs in
	...
	let nt_sigs' =
		...
		BatSet.filter (fun nt_sig ->
			...
			List.for_all (fun (nt_const, desired_const) ->
				(Stdlib.compare nt_const desired_const) = 0
			) (BatList.combine nt_sig desired_sig)		
		) nt_sigs 
	in
	(* if we have SynthLang.Exprs covering all the points, pick the smallest one *)
	if not (BatSet.is_empty nt_sigs') then
		BatSet.elements nt_sigs'
		|> List.map (fun nt_sig -> BatMap.find (nt, nt_sig) nt_sig_to_expr)
		|> List.sort (fun e1 e2 -> (SynthLang.Exprs.size_of_expr e1) - (SynthLang.Exprs.size_of_expr e2))
		|> List.hd 
	else
